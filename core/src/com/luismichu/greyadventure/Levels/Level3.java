...
public class Level3 extends Level {
    ...
    private final OrthographicCamera cameraTiled;
    private final MyCustomContactListener contactListener;
    private final Sound deathSound;
    private final Array<Sound> deathPlayerSound;
    private final RayHandler rayHandler;
    private final PointLight lightEnd;

    public static Level3 create(MyAssetManager assetManager, MyPreferenceManager preferenceManager, MyPhysicManager physicManager,
                                OrthographicCamera camera, OrthographicCamera cameraUI, GameState gameState){
        level3 = new Level3(assetManager, preferenceManager, physicManager, camera, cameraUI, gameState);
        return getInstance();
    }
    ...
    private Level3(MyAssetManager assetManager, MyPreferenceManager preferenceManager, MyPhysicManager physicManager,
                   OrthographicCamera camera, OrthographicCamera cameraUI, GameState gameState){
        super(assetManager, preferenceManager, physicManager, camera, cameraUI, gameState);

        assetManager.addToQueue(MyAssetManager.AssetDescriptors.map3);
        assetManager.addToQueue(MyAssetManager.AssetDescriptors.enemyBlueAttack);
        ...
        assetManager.addToQueue(MyAssetManager.AssetDescriptors.musicLevel3);
        ...
        map = (TiledMap) assetManager.get(MyAssetManager.AssetDescriptors.map3);
        ...
        createEnemies(assetManager.getTextures(MyAssetManager.AssetDescriptors.enemyBlueAttack));
        createDialogs();
        ...
        music = assetManager.getMusic(MyAssetManager.AssetDescriptors.musicLevel3);
            ...
            if(fading){
                ...
                if (contactListener.isDead() && canDie || grey.isDead()) {
                    ...
                    if (contactListener.isDamaged()) {
                        if(grey.kill()){
                            Enemy killedEnemy = enemies.get(contactListener.enemy);
                            if(!killedEnemy.dead) {
                                garbageBodies.add(killedEnemy.getBody());
                                killedEnemy.kill();
                                deathSound.play(preferenceManager.getVolume());
                            }
                        }
                        else
                            grey.damage();
